...
function wordpress_cta_get_design_templates() {
	$styles = array(
		'fitness1' => [
			'name' => 'Fitness',
			'default_image' => EASY_STICKY_SIDEBAR_PLUGIN_DIR . '/assets/img/fitness.jpg',
		...
		'wp_cta_pro' => array(
			'name' => 'WP CTA PRO',
			'default_image' => EASY_STICKY_SIDEBAR_PLUGIN_DIR . '/assets/img/wp-cta-pro.png',
	...
	array_walk($styles, function(&$style, $key) use($design_template_images){
		unset($style['sidebar_name']);
		$key = sanitize_title( $key );
		$attach_id = @$design_template_images[$key];
		if ( !wp_get_attachment_image_url($attach_id) ) {
			return;
		}

		$style['image_attachment_id'] = $attach_id;
		$style['sticky_s_media'] = wp_get_attachment_image_url($attach_id, 'full');		
	});
